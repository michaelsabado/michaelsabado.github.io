...
<template>
  ...
  >
    ...
    <div class="relative space-y-10">
      <span
        class="pointer-events-none absolute left-4 top-0 hidden h-full w-px bg-slate-200 md:block dark:bg-slate-800"
      ></span>

      <article
        v-for="company in workExperience"
        :key="company.company"
        class="relative rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-card transition-all md:pl-14 dark:border-slate-800/80 dark:bg-slate-900/70"
      >
        <span
          class="pointer-events-none absolute left-3 top-8 hidden h-4 w-4 rounded-full border-4 border-white bg-brand-500 shadow-[0_0_0_8px_rgba(14,165,233,0.2)] md:block dark:border-slate-900"
        ></span>
